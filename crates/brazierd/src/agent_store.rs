...
use crate::{
    ...
    agent_types::{
        ...
        CreateAgentSession, SandboxDescription, ToolExecutionRecord, ToolRiskLevel,
        UpdateAgentSession,
    },
    db::Database,
};
...
impl Database {
    ...
    pub async fn decide_approval(
        ...
        if approved && effective_scope == ApprovalScope::Session && !current.scope_key.is_empty() {
            let key = crate::agent_policy::grant_key(current.environment, &current.scope_key);
            sqlx::query(
...
pub fn unknown_sandbox() -> SandboxDescription {
    SandboxDescription {
        ...
        profile: SandboxProfile::Workspace.as_str().to_owned(),
        isolated: false,
        network: false,
        workspace_path: None,
...
mod tests {
    use super::*;
    use crate::agent_types::{RequestedPathAccess, arguments_hash};
    use serde_json::json;
