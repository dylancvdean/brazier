...
use crate::{
    ...
    agent_store::{NewApproval, NewToolExecution},
    agent_types::{
...
async fn bound_output(
    ...
    let head: String = output.chars().take(MAX_MODEL_OUTPUT_CHARS / 2).collect();
    let tail_start = output
        .chars()
        .count()
        .saturating_sub(MAX_MODEL_OUTPUT_CHARS / 2);
    let tail: String = output.chars().skip(tail_start).collect();
    let dropped = output.chars().count() - head.chars().count() - tail.chars().count();
...
mod tests {
    use super::*;
    use crate::{
        agent_types::{AgentPermissionMode, AgentPermissionSettings, CreateAgentSession},
        db::Database,
    };
    use serde_json::json;
