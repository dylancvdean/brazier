//! Model Context Protocol (MCP) client and server configuration.
//!
...
pub async fn save(data_dir: &Path, config: &McpConfig) -> anyhow::Result<()> {
    ...
    let bytes = serde_json::to_vec_pretty(config).context("encode MCP config")?;
    tokio::fs::write(&temporary, bytes)
        .await
        .context("write MCP config")?;
    tokio::fs::rename(&temporary, &path)
        .await
        .context("commit MCP config")?;
    Ok(())
}
