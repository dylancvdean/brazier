...
use zbus::{Connection, Proxy, zvariant::{OwnedObjectPath, OwnedValue, Str}};

const DESKTOP: &str = "org.freedesktop.portal.Desktop";
const ROOT: &str = "/org/freedesktop/portal/desktop";

struct PortalSession { connection: Connection, path: OwnedObjectPath, stream: u32 }
...
async fn request_response(connection: &Connection, request: OwnedObjectPath) -> Result<HashMap<String, OwnedValue>, String> {
    let request = proxy(connection, request.as_str(), "org.freedesktop.portal.Request").await?;
    ...
    let (code, results): (u32, HashMap<String, OwnedValue>) = message.body().deserialize().map_err(|e| e.to_string())?;
    match code { 0 => Ok(results), 1 => Err("Desktop portal request was cancelled by the user.".into()), _ => Err(format!("Desktop portal request failed (response code {code}).")) }
}

async fn response_from(mut responses: zbus::proxy::SignalStream<'_>) -> Result<HashMap<String, OwnedValue>, String> {
...
fn expected_request_path(connection: &Connection, token: &str) -> Result<String, String> {
    let sender = connection.unique_name().ok_or("session D-Bus did not assign Brazier a unique name")?
        .as_str().trim_start_matches(':').replace('.', "_");
...
fn string_value(value: &str) -> OwnedValue { OwnedValue::from(Str::from(value)) }
...
async fn response_listener<'a>(connection: &'a Connection, token: &str) -> Result<zbus::proxy::SignalStream<'a>, String> {
    let path = expected_request_path(connection, token)?;
    proxy(connection, &path, "org.freedesktop.portal.Request").await?
        .receive_signal("Response").await.map_err(|e| e.to_string())
}
fn value<T: TryFrom<OwnedValue>>(results: &mut HashMap<String, OwnedValue>, key: &str) -> Result<T, String> {
    results.remove(key).ok_or_else(|| format!("desktop portal response omitted {key}"))?.try_into()
...
pub async fn screenshot() -> Result<Vec<u8>, String> {
    let connection = Connection::session().await.map_err(|e| format!("connect to session D-Bus: {e}"))?;
    let screenshot = proxy(&connection, ROOT, "org.freedesktop.portal.Screenshot").await?;
    let request: OwnedObjectPath = screenshot.call("Screenshot", &("", options())).await.map_err(|e| e.to_string())?;
    let mut results = request_response(&connection, request).await?;
    let uri: String = value(&mut results, "uri")?;
    let path = uri.strip_prefix("file://").ok_or("desktop portal returned a non-file screenshot URI")?;
    tokio::fs::read(path).await.map_err(|e| format!("read portal screenshot: {e}"))
}
...
async fn create_session() -> Result<PortalSession, String> {
    ...
    let responses = request_proxy.receive_signal("Response").await.map_err(|e| e.to_string())?;
    let request: OwnedObjectPath = remote.call("CreateSession", &create_options).await.map_err(|e| e.to_string())?;
    if request.as_str() != expected {
        return Err(format!("desktop portal returned unexpected request handle {}", request.as_str()));
    }
    ...
    let path = OwnedObjectPath::try_from(path).map_err(|error| format!("invalid portal session handle: {error}"))?;
    ...
    let request: OwnedObjectPath = screencast.call("SelectSources", &(path.clone(), source_options)).await.map_err(|e| e.to_string())?;
    if request.as_str() != expected_request_path(&connection, &source_token)? { return Err("desktop portal returned an unexpected SelectSources handle".into()); }
...
async fn with_session<T>(operation: impl FnOnce(&PortalSession) -> T) -> Result<T, String> {
    ...
    if guard.is_none() { *guard = Some(create_session().await?); }
...
async fn remote_call<T: serde::Serialize + zbus::zvariant::DynamicType>(method: &str, body: &T) -> Result<(), String> {
    let (connection, _) = with_session(|s| (s.connection.clone(), s.path.clone())).await?;
    let remote = proxy(&connection, ROOT, "org.freedesktop.portal.RemoteDesktop").await?;
    remote.call::<_, _, ()>(method, body).await.map_err(|e| e.to_string())
}

pub async fn pointer_motion(x: f64, y: f64) -> Result<(), String> {
    let (path, stream) = with_session(|s| (s.path.clone(), s.stream)).await?;
    remote_call("NotifyPointerMotionAbsolute", &(path, options(), stream, x, y)).await
}
...
pub async fn scroll(delta_x: f64, delta_y: f64) -> Result<(), String> {
    let path = with_session(|s| s.path.clone()).await?;
    if delta_y != 0.0 { remote_call("NotifyPointerAxis", &(path.clone(), options(), 0_u32, delta_y)).await?; }
    if delta_x != 0.0 { remote_call("NotifyPointerAxis", &(path, options(), 1_u32, delta_x)).await?; }
    Ok(())
}
pub async fn key(keysym: u32) -> Result<(), String> {
    let path = with_session(|s| s.path.clone()).await?;
    let keysym = i32::try_from(keysym).map_err(|_| "invalid keyboard keysym")?;
    remote_call("NotifyKeyboardKeysym", &(path.clone(), options(), keysym, 1_u32)).await?;
    remote_call("NotifyKeyboardKeysym", &(path, options(), keysym, 0_u32)).await
}
pub async fn type_text(text: &str) -> Result<(), String> {
    for character in text.chars() { key(if character.is_ascii() { character as u32 } else { 0x0100_0000 | character as u32 }).await?; }
    Ok(())
}
pub async fn close_session() {
    let Some(session) = session_slot().lock().await.take() else { return };
    if let Ok(portal_session) = proxy(&session.connection, session.path.as_str(), "org.freedesktop.portal.Session").await {
